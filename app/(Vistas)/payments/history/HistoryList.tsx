    ...
    <div className="p-4 min-[600px]:p-5 lgx:p-6">
      ...
      <Card padding={0}>
        {transactions.map((t, i) => {
          ...
          const iconBg = failed
            ? "bg-[#f0d9d1] text-danger"
            : pending
              ? "bg-[#f3e4c4] text-warn"
              : isPos
                ? "bg-[#dde2c9] text-success"
                : "bg-bone text-ink-2";
          const amountCls = failed
            ? "text-ink-3"
            : pending
              ? "text-warn"
              : isPos
          ...
          return (
            <div
              key={t.id}
              className={i < transactions.length - 1 ? "border-b border-line" : ""}
            >
              <div
              ...
              >
                ...
                >
                  <Icon
                    name={failed ? "close" : pending ? "clock" : isPos ? "arrowDown" : "arrowUp"}
                    size={16}
                  />
                </div>
                <div className="flex-1 min-w-0 pr-2">
                  <div className="text-[13px] font-medium truncate" title={t.desc}>
                    {t.desc}
                  </div>
              ...
              {isExpanded && (
                <div className="px-4 pb-4 pt-3 bg-bone/30 border-t border-line">
                  <div className="text-[11px] font-semibold text-ink-3 mb-3 uppercase tracking-wider">
                    Detalle de la orden
                  </div>
                      ...
                      <div key={j}>
                        <div className="flex justify-between items-center bg-white p-2.5 rounded-lg shadow-sm border border-line/50 gap-2">
                          <span className="text-[13px] text-ink font-medium flex items-center gap-2 min-w-0 flex-1">
                            <span className="bg-bone text-ink-2 px-1.5 py-0.5 rounded text-[11px] font-bold shrink-0">
                              {item.quantity}x
                            </span>
                        ...
                        {item.shippingPrice > 0 && (
                          <div className="flex justify-between items-center py-1 px-2">
                            <span className="text-[12px] text-ink-3">Envio</span>
                            <span className="text-[12px] text-ink-3">
                              {fmtARS(item.shippingPrice)}
                            </span>
                          </div>
                        )}
                      </div>
